...
class MultipleKalmanDerivator:
    ...
    """
    def __init__(self, deltaT, x0, dx0, ddx0, n_obs=1, sigmaR = 1e-3, sigmaQ = 1e-1):
        # initial conditions
        self.q = copy(x0) # [x, y, z]
        self.dq = copy(dx0)  # [dx, dy, dz]
        self.ddq = copy(ddx0)  # [ddx, ddy, ddz]
    ...
    def update(self, ddq_med, new_deltaT=None):
        # update the kalman filter for each degree of freedom
        for i in range(self.n_dof):
            q, dq, ddq = self.derivators[i].run_kalman_filter(ddq_med[i], new_deltaT=new_deltaT)
            self.q[i] = q
            self.dq[i] = dq
            self.ddq[i] = ddq
...
class KalmanDerivator:
    ...
    def __init__(self, x_est0, n_obs, deltaT, sigmaR = 1e-3, sigmaQ = 1e-1):
        ...
        self.F = np.array([[1., self.deltaT, self.deltaT**2/2],[0., 1., self.deltaT],[0.,0.,1.]]) # model
        ...
        self.H = np.array([0,0,1]).reshape((self.n_obs, self.n_input)) # to scale measurements
    ...
    def run_kalman_filter(self, ddq, new_deltaT=None):
