...
_GOALS ={'state':True, 'upper':np.deg2rad(100), 'lower':np.deg2rad(10)}
...
class ExerciseEnv(object):
    ...
    def get_goal_angle(self,metric):
        ...
        if self._sim_timesteps % (self._max_sim_timesteps/4) ==0 and self._sim_timesteps != 0 and not self.first_time:
            ...
            new_goal_angle =  _GOALS['upper'] if _GOALS['state'] else  _GOALS['lower']

            self.update_bullet_position(user_parameters=self.user_parameters, goal_angle=new_goal_angle)

        elif abs(metric)<np.deg2rad(10) and self.first_time:
